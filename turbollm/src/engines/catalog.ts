...
const ALL: CatalogEngine[] = [
  ...
  // ── Awareness-only engines (comingSoon: listed with pros/cons + hardware fit, but
  // not yet installable from TurboLLM — no provisioning path wired). Each needs its own
  // engine `kind` + launch/readiness/OpenAI-compat wiring before the flag can be removed
  // (that work is gated on the EngineKind descriptor refactor). Their `variants` exist only
  // so the recommender can compute a real hardware-fit for the card's compatibility mark.
  {
    id: 'exllamav3',
    name: 'ExLlamaV3 + TabbyAPI',
    kind: 'exllama',
    description:
      'GPU inference for consumer NVIDIA cards using EXL3 quantization — strong quality at 2–3 bits. Served through TabbyAPI’s OpenAI-compatible API.',
    provision: 'pip',
    ...
    installEndpoint: '',
    comingSoon: true,
    note: 'Not yet installable from TurboLLM. NVIDIA (CUDA) on Windows/Linux; uses EXL3/EXL2 quants, not GGUF.',
    variants: [
      {
        id: 'exllamav3-cuda',
        label: 'CUDA (NVIDIA)',
        repo: 'turboderp-org/exllamav3',
        requires: { platform: ['win32', 'linux'], gpuVendor: ['nvidia'] },
        stability: 'experimental',
        speed: 'fast',
        hasPrebuilt: false,
      },
  ...
  },
  {
    ...
    installEndpoint: '',
    comingSoon: true,
    note: 'Not yet installable from TurboLLM. macOS (Apple Silicon) only; runs MLX-format models.',
    variants: [
      {
        id: 'rapid-mlx-metal',
        label: 'Apple Silicon',
  ...
  },
  {
    ...
    kind: 'nexa',
    description:
      'On-device engine spanning CPU, GPU and NPU (incl. Copilot+ / Snapdragon) with an OpenAI-compatible server. Runs GGUF, MLX and its own format.',
    provision: 'pip',
    homepage: 'https://github.com/NexaAI/nexa-sdk',
    repo: 'NexaAI/nexa-sdk',
    platforms: ['win32', 'darwin', 'linux'],
    support: 'experimental',
    installEndpoint: '',
    comingSoon: true,
    note: 'Not yet installable from TurboLLM. Unique NPU coverage (Qualcomm/Intel/Apple); the NPU path needs a free Nexa license token.',
    variants: [
      {
        id: 'nexa-any',
        label: 'CPU / GPU / NPU',
  ...
  },
  {
    ...
    kind: 'ktransformers',
    description:
      'CPU/GPU hybrid engine that runs very large MoE models (DeepSeek-class) on a single consumer GPU plus system RAM. OpenAI-compatible.',
    provision: 'pip',
    ...
    installEndpoint: '',
    comingSoon: true,
    note: 'Not yet installable from TurboLLM. Build-from-source, Linux-first; best speed on Intel AMX CPUs with lots of RAM.',
    variants: [
      {
        id: 'ktransformers-cuda',
        label: 'CUDA + CPU hybrid',
        repo: 'kvcache-ai/ktransformers',
        requires: { platform: ['linux', 'win32'], gpuVendor: ['nvidia'] },
        stability: 'experimental',
        speed: 'fast',
        hasPrebuilt: false,
      },
  ...
  },
  {
    ...
    kind: 'lmdeploy',
    description:
      'High-throughput NVIDIA serving (TurboMind C++/CUDA) with strong AWQ / MXFP4 / FP8 quant support and an OpenAI-compatible server.',
    provision: 'pip',
    ...
    installEndpoint: '',
    comingSoon: true,
    note: 'Not yet installable from TurboLLM. NVIDIA (CUDA) only; loads HF / AWQ weights, not GGUF.',
    variants: [
      {
        id: 'lmdeploy-cuda',
        label: 'CUDA (NVIDIA)',
  ...
  },
  {
    ...
    kind: 'tensorrt-llm',
    description:
      "NVIDIA's peak-throughput engine with an OpenAI-compatible server (trtllm-serve). Datacenter-grade; heavyweight setup.",
    provision: 'pip',
    ...
    installEndpoint: '',
    comingSoon: true,
    note: 'Not yet installable from TurboLLM. NVIDIA + Linux (Docker); loads HF checkpoints, not GGUF.',
    variants: [
      {
        id: 'tensorrt-cuda',
        label: 'CUDA (NVIDIA, Linux)',
        repo: 'NVIDIA/TensorRT-LLM',
        requires: { platform: ['linux'], gpuVendor: ['nvidia'] },
        stability: 'experimental',
        speed: 'fastest',
