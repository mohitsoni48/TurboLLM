...
const UVLOOP_WHEEL_PLATFORMS: readonly NodeJS.Platform[] = ['linux', 'darwin']
...
export class UvloopPreflight {
  ...
  private brokenEnvironment(platform: NodeJS.Platform, failure?: PythonProbeFailure): string {
    return (
      ...
      `(uvloop failed to import: ${failureDetail(failure)}). ${this.platformStanding(platform)}`
    )
  }

  private platformStanding(platform: NodeJS.Platform): string {
    const plat = platformDisplay(platform)
    if (UVLOOP_WHEEL_PLATFORMS.includes(platform)) {
      return (
...
function failureDetail(failure?: PythonProbeFailure): string {
  const raw = failure?.stderr ? String(failure.stderr) : (failure?.message ?? '')
  return capped(withHomeRedacted(lastMeaningfulLine(raw)))
}
...
function withHomeRedacted(text: string): string {
  ...
  return [...spellings].reduce((redacted, spelling) => redacted.split(spelling).join('~'), text)
}
