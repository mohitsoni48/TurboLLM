...
test('buildSaveSkillTool: grants exactly save_skill', () => {
  ...
  assert.deepEqual([...ts.names], ['save_skill'])
})

test('buildSaveSkillTool: save_skill routes to the callback with parsed args', () => {
  let received: unknown
  const ts = buildSaveSkillTool((args) => { received = args; return 'saved' })
  const r = ts.execute({ id: '1', name: 'save_skill', args: { name: 'csv-summary', description: 'Summarize a CSV', procedure: 'Read then summarize.' } })
  assert.equal(r, 'saved')
  assert.deepEqual(received, { name: 'csv-summary', description: 'Summarize a CSV', procedure: 'Read then summarize.' })
})

test('buildSaveSkillTool: unknown tool rejected by executor', () => {
  const ts = buildSaveSkillTool(() => 'ok')
