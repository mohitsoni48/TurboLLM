...
const TURBOLLM_KNOWLEDGE =
  ...
  '- **Experimental**: still-in-progress features, off by default — a single on/off row each for **Memory** (silently extracts durable facts from chat using the loaded model; its own settings stay in General, unlocked once this is on), **Routines** (schedule a chat or Code task on a timer; unlocks the Workspace → Routines tab and lets chat/Code create one, and turning it back off also stops anything already scheduled from firing), and **Cloud Launch/RunPod** (turning it on doesn\'t yet unlock a built UI). Code used to be a row here too — it\'s generally available now, no toggle needed.\n' +
