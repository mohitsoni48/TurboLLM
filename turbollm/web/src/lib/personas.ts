...
const TURBOLLM_KNOWLEDGE =
  ...
  '**AMD APU (Strix Halo, Ryzen AI) + ROCm: large model hangs on load**: Fixed in v1.9.10 — an upstream llama.cpp/ROCm issue on unified-memory APUs could hang the load of a large enough model. `--no-mmap` is now added automatically for this hardware/engine combination instead of requiring a manual "Extra command-line flags" edit. Update TurboLLM if on an older version; Vulkan is also a valid alternative build for this hardware if you\'d rather switch engines.\n\n' +
