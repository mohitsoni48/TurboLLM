...
const TURBOLLM_KNOWLEDGE =
  ...
  '- **System**: hardware panel, **Privacy & telemetry** — three levels (Off / anonymous usage + benchmarks / + crash reports), no pre-selected default, asked once on first run and changeable here any time; "Preview what we send" shows the exact payload, "What was actually sent" is a local log of every event this machine has really transmitted, plus a button to regenerate the anonymous machine ID — and About (current version, update-available chip, copy install command).\n\n' +
