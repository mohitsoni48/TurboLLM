...
const TURBOLLM_KNOWLEDGE =
  ...
  '- **Payoff**: Casual/Enthusiast start a real conversation; Developer saves their picked coding-agent default (turbollm, claude, pi or opencode) and is handed to the real Code launchpad (repo picker, model picker, task composer) to start their first task — never a fabricated session. One click completes setup and lands you there.\n' +
  ...
  '- **Reasoning effort**: for a model whose own chat template supports it (e.g. Qwen3.8), the thinking-budget slider is automatically replaced by an Off/Low/Medium/xhigh reasoning-effort slider instead — detected per model, not user-configured. Same swap happens in Code, including a `claude`/other external-CLI session.\n\n' +
  ...
  '- Point it at a repo folder, describe a task, pick a mode (plan-only / ask-before-mutating / auto plan-and-edit) — it reads, edits files, runs shell commands, and reports a real diff.\n' +
  '- **Use worktree** (a toggle in the composer): runs the session in a real isolated git worktree on its own branch, created at `<repo>/.turbollm/worktrees/<branch>`, so your actual checkout and current branch are never touched — `git status` in your repo stays clean while the agent works. The branch name is derived from the task (editable), and a name that is already taken is suffixed rather than reused. Everything the session does happens in that worktree: the agent\'s own edits, the `!command` shell escape, and the Commit / Push / diff actions. Deleting the session removes the worktree, but never destroys work — if it still has uncommitted changes the removal is refused and you are told the exact `git worktree remove` command to finish; committed work always survives, because removing a worktree does not delete its branch. Requires the folder to be a git repository; if it is not, the session tells you so instead of silently working in the folder directly.\n' +
  ...
  '- **Run an external CLI agent instead of the built-in one**: Settings → Code agent switches new sessions between `turbollm` (the built-in agent above) and an external CLI — `claude`, `pi` or `opencode`. An agent that is not installed is listed but not selectable, with an Install button that runs its own install command for you. A CLI launched this way never inherits your cloud provider API keys (a TurboLLM session is a local-model session), and an `opencode` session is offered only TurboLLM\'s models. Each CLI\'s own model picker lists your whole library with the real context window each model will load with. A `claude` session opens the CLI in a full-screen terminal inside Code, pointed at your local model and your project folder — the model picker, context ring, thinking-budget slider and stats row stay around it (the CLI replaces the transcript, not the controls). It starts in the mode the session was created with (auto / plan / ask), the thinking-budget slider applies live per request with no restart, switching models drives the CLI\'s own /model command so scrollback survives, and the conversation resumes after a TurboLLM restart. Ctrl+D leaves the terminal without killing the agent. The choice is read once per session at creation, so changing it only affects new sessions. Your first message in a new session is sent to the CLI automatically (no need to retype it); web search and page fetches work and are pre-approved in Auto mode only (Plan and Ask still ask); for `pi` specifically the research tools (web search, code search, fetch) ship by default — TurboLLM installs the `pi-search` package best-effort on first launch, announces it once, and you can opt out with `TOBOLLM_PI_DISABLE_SEARCH_INSTALL=1`; and the CLI follows the same working rules as the built-in agent — research before retrying, check a package\'s current version and real docs before adding a dependency, search the web after two failures in a row, and stop rather than loop on the same tool call. Its background agents are capped at the number of requests the loaded engine can serve at once, with the rest queued. The terminal backend is an optional native component: if it could not be installed on this machine, `claude` is simply not listed in the picker (and a previously-saved `claude` setting is shown with a note explaining why) — TurboLLM itself installs and runs normally either way.\n' +
  '- **Stats row under a Code session** reads `Think <budget> · Context <%> of <max> · Last turn ↑<prompt> ↓<generated> · <n> tok/s` (or `Reasoning <Off/Low/Medium/xhigh>` in place of `Think` for a model with a reasoning-effort control), identical for the built-in agent and a CLI one. The tok/s figure is the engine\'s own measured generation rate, not throughput averaged over the whole round trip.\n\n' +

  '**Routines** (Workspace → Routines tab, next to Chat/Code — **experimental, off by default**: turn it on in Settings → Experimental first, or the tab/nav badge stay hidden and chat/Code both refuse to create one) — schedule a chat or Code task to run on its own, on a timer, with no one present.\n' +
  '- Pick "chat" (runs through a Customize → Agents persona) or "code" (in-app agent or the Claude Code CLI, against a real workspace path) and a schedule (interval / daily / weekly).\n' +
  '- Always starts "pending confirmation" and never fires until you confirm it in the panel — creating one, including via chat or Code asking on your behalf, is never itself enough to arm it.\n' +
  '- Pause/resume any time, "Run now" to trigger immediately, and a full run history per routine (result, error, or a live tool-call approval card when a run needs your OK mid-flight).\n' +
  '- Turning the experimental flag off later stops everything already scheduled from firing too, not just new ones from being created.\n\n' +
  ...
  '- **Presets** (top of Model Detail): a model can keep MANY named load configs, not just one. Pick one from the dropdown to fill the load parameters — the pick is remembered and auto-applied on the next load of that model. **Save as…** captures the current settings as a new preset; the ⋯ menu renames or deletes. Auto-tune mints a preset on every save, named `Auto-tune <date>` and badged with its measured tok/s, so tuning accumulates history instead of overwriting. Retention keeps the 10 newest auto-tune presets and never prunes presets you made yourself. Each preset records the engine it was tuned on; picking one that belongs to a different engine shows a note and does not override that engine\'s own saved tuning — switch engines first. Existing saved profiles appear as presets automatically on first load, and a model with no presets behaves exactly as before.\n' +
  ...
  '**Engines** — manage inference backends. Three tabs (issue #211 follow-up, superseding the 8-icon nav rail): **Engines** (below), **Monitor** (live engine log + a developer request log, see the dedicated ## Monitor section), and **Usage** (the token dashboard, formerly its own nav icon — see below).\n' +
  ...
  '- **Connect tab** — point an outside app or CLI at this server, or expose Code to one. Not to be confused with the MCP Servers tab above: this is TurboLLM acting AS a server/MCP host, not calling one.\n' +
  ...
  '  - **TurboLLM MCP section** ("TurboLLM MCP" heading, distinct from the MCP Servers tab\'s "MCP Servers" heading): setup for `turbollm mcp-server`, a stdio MCP server exposing Code as a `delegate_code_task` tool any MCP-compatible tool can call (Claude Desktop, Cursor, Windsurf, Cline, Claude Code, etc.) — the reverse direction from the MCP Servers tab.\n' +
  '  - **Key management is host-only while the LAN is open and unauthenticated** (LAN sharing on, "Require an API key" off in Settings → Network): viewing/creating/revoking keys and the connect-setup snippets (which embed a live key) are locked to this machine until a key is required, with an explanatory message instead of just silently missing. Once a key **is** required, a verified key on a non-host device can drive the host-execution build/engine routes (build/run, build/cuda, install prerequisites, add engine, scan engine) — but an open, keyless LAN still cannot trigger any of them, and a Turbo Link token is structurally excluded from them regardless.\n\n' +

  '**Usage** — a token-usage dashboard sourced from message-level stats already persisted per turn (no separate opt-in), now a TAB on Engines (issue #211 follow-up) rather than its own nav icon.\n' +
  ...
  '- **General**: theme (light/dark/system), enable-thinking-by-default, confirm-before-delete, personalization (assistant name / your name), **Memory** — its toggle now lives in Experimental (see below); when on, the reviewable/deletable fact list still shows here, auto-generate chat titles, open browser on start, and **Show hardware monitor** — toggles the persistent bottom status bar (CPU/RAM/GPU/VRAM values and percentages) that appears on every screen.\n' +
  '- **Models & loading**: idle timeout (auto-stop after N minutes), default context length, Gateway (auto model-swap + Keep-N pool, 1–4 models), model folders, Hugging Face token, GitHub token (raises the 60/hour anonymous GitHub API limit to 5,000 — used for build-from-source branch lists, engine update checks, and engine downloads; write-only, applies immediately), and an **Advanced** collapsible for expert knobs — default GPU layers, VRAM headroom (300 MB–2 GB, default 1 GB, or drag to 0 to opt into MoE auto-tune\'s "VRAM-spill" search — see Auto-Tune below), and image/response token caps.\n' +
  ...
  '- **Experimental**: still-in-progress features, off by default — a single on/off row each for **Memory** (silently extracts durable facts from chat using the loaded model; its own settings stay in General, unlocked once this is on), **Routines** (schedule a chat or Code task on a timer; unlocks the Workspace → Routines tab and lets chat/Code create one, and turning it back off also stops anything already scheduled from firing), **Turbo Link** (link a second machine running TurboLLM and use its models as if they were local: they appear in the chat model picker grouped by machine, in /v1/models and in launch --model, and Models/Downloads/Engines merge both machines\' rows into one list, each row labelled with the machine it lives on and unavailable actions disabled with the reason shown; you paste a link string minted on the other machine, granting an explicit capability set — three presets: use models only / run a server / full control, plus Customize — optionally pinned to an allowlist of specific models and revocable at any time from the host panel; links never chain, and a generation that runs on a linked host counts as that host\'s work in its own stats. Not yet verified against a real two-machine setup, which is why it is experimental; while the toggle is off the settings section, the remote rows and every link route stay hidden), and **Cloud Launch/RunPod** (turning it on doesn\'t yet unlock a built UI). Code used to be a row here too — it\'s generally available now, no toggle needed.\n' +
  '- **System**: hardware panel (static specs), **live hardware gauges** with ~60 s SVG sparklines (real-time CPU/RAM/GPU/VRAM from nvidia-smi, WDDM, rocm-smi, AMD sysfs, ioreg, or /proc), **Privacy & telemetry** — three levels (Off / anonymous usage + benchmarks / + crash reports), defaults to the full level with no first-run prompt, changeable here any time; "Preview what we send" shows the exact payload, "What was actually sent" is a local log of every event this machine has really transmitted, plus a button to regenerate the anonymous machine ID — and About (current version, and since v1.13.0 an "Update & restart" button when a newer TurboLLM is out — applies the update in place for a global npm/npx install, no terminal needed; other install methods still fall back to a copyable command). The same update control also lives at the bottom of the nav rail, right where the version number is shown — click it there too. An off/notify/auto policy under About controls whether updates are only surfaced or applied automatically.\n\n' +
  ...
  '**Phase 1 — Offload search** (load probes, no generation), on whatever KV-cache type you already have selected — auto-tune no longer picks the KV type for you. Each probe checks TWO things: free VRAM against the configured headroom, AND how much memory the load actually pushed into system RAM ("spill"). Spill matters because a GPU driver does not fail when a model does not fit — it silently keeps the overflow in system RAM, and reported VRAM usage pins at the card ceiling and stops changing. A config spilling more than ~512 MB is rejected even when VRAM usage looks fine, because that config would run partly from system RAM (slow, and sometimes crash-prone). On machines where the OS does not expose that figure (Apple Silicon, and Linux+NVIDIA where CUDA fails the allocation instead of spilling) the spill check simply sits out and the search behaves as it always did. On a multi-GPU machine the search also places the layers itself rather than accepting a `--tensor-split` saved on the profile: it starts from an even baseline and, when that would overflow a card, derives a split by layer SIZE (MoE layers whose experts moved to CPU are ~10x lighter, so an even layer COUNT is not an even byte split). A split you pinned yourself is ignored for the duration of the sweep — it would otherwise cap how much VRAM the tuner can reach — but it is never changed on disk unless you Save the tuned result.\n' +
  '- Dense models: search ngl ∈ [0, blockCount] — finds highest ngl that neither exceeds the configured VRAM headroom buffer (Settings → Models & loading → Advanced, default 1 GB, adjustable 300 MB–2 GB) nor spills.\n' +
  '- MoE models: search nCpuMoe ∈ [0, nExpertsTotal] — ngl stays maxed, finds minimum nCpuMoe (router experts on CPU) that fits the headroom without spilling. Reducing nCpuMoe pushes more MoE routing onto the GPU. If VRAM headroom is set to 0 (an explicit opt-in, not the default), MoE models then hill-climb PAST that safe point — pushing nCpuMoe lower still, for as long as each further step measures a faster time-to-answer.\n' +
  ...
  'One real prefill + generation run. Bench prompt: `min(50,000 tokens, ctx × 75%)`. Per-test cap: 10 minutes. Stop/restart/load cancel a running auto-tune. Records prefill t/s, generation t/s, TTFT ms, and VRAM delta. If this run crashes or times out, auto-tune steps back to the next safer offload setting and retries rather than ending with no result. The winner is chosen by TIME TO A COMPLETE ANSWER (time-to-first-token plus generating 1,000 tokens), not by generation speed alone — generation speed keeps rising as more of a model is forced onto the GPU, including past the point where it starts spilling, so optimizing for it alone favours configs that are slower in real use.\n\n' +
  ...
  '- Speculative decoding (`speculative`: off / mtp / nextn / draft / dflash): `mtp` uses a separate Gemma-style MTP head (`mtpHeadPath`); `nextn` self-speculates off the model\'s own built-in NextN head (Qwen3-family, free — no extra file); `draft` uses a separate small draft GGUF (`draftModelPath`); `dflash` is adaptive DFlash speculative decoding with a separate draft GGUF — chosen from a library dropdown in the model dialog, shown only when the engine reports `spec-type:draft-dflash` support. All modes honor configurable `draftMax`/`draftMin` (tokens drafted per step, default 16/1) — the draft-window fields apply to whichever speculative mode is active, not just `draft`. A well-matched draft/main pair can 2–4× throughput.\n\n' +
  ...
  '- `loadMode` (`--load-mode`): how the weights are read off disk — `dio` uses direct I/O and skips the OS page cache, the usual pick when system RAM is tight. Shown only on engine builds that advertise the flag, and the offered values come from that engine\'s own `--help`, so a fork\'s extra mode appears automatically. Leave on "engine default" unless loading misbehaves. Older builds spell the same idea `-dio` (single-dash), which is still reachable through `extraArgs`.\n' +
  '- `noMmap` (`--no-mmap`): read the model into RAM instead of memory-mapping the file. Slower to load, but avoids page-cache thrashing — and it is the known fix for the load hang on AMD unified-memory APUs (which TurboLLM also applies automatically for that hardware; see Troubleshooting).\n' +
  '- `cacheRam` (`--cache-ram`): MiB of RAM llama.cpp may use to keep finished prompts, so returning to an earlier conversation skips re-processing. Blank/absent = engine default (8192 MiB, prompt cache on); 0 turns the prompt cache off and gives that RAM back. Shown only on engine builds that advertise the flag. Nothing changes until the user sets it.\n' +
  '- `extraArgs`: free-text custom/raw command-line flags, appended to the launch command last so they can override anything above. For llama.cpp/vLLM engines. Type a flag and its value together (e.g. `--lora adapter.gguf`) and press Enter — they are split into separate arguments for you; quote a value that itself contains spaces. Repeating a flag is allowed.\n' +
  ...
  '## Monitor\n\n' +
  'A TAB on Engines (rail order: Workspace, Models, Engines, Customize, Settings — 5 icons since issue #211\'s follow-up, down from the original 8) that puts the live engine log, a developer request log, and system hardware stats on one always-visible, split screen — no more opening the Engines drawer or jumping to Settings → System to see them. Top half is a segmented control between two DIFFERENT captures:\n' +
  '- **Engine log**: the same log/auto-scroll behavior as the Engines diagnostics drawer (shows the last-loaded engine\'s log even after it stopped, gated on an engine being selected, not necessarily running). This is the engine\'s own stderr — timing/slot lines only (e.g. llama.cpp\'s `slot print_timing`); it never contains a prompt, a response, or a sampling parameter like temperature, because the engine itself never emits them.\n' +
  '- **Requests** (issue #211 follow-up: "the log needs to show the actual request, response, and every sampling parameter, like LM Studio"): a SEPARATE capture, sourced from TurboLLM\'s own proxy layer (the gateway and the in-app chat upstream) rather than the engine — so it works identically across every engine, and sees every completion this daemon serves: external API clients (Claude Code, opencode, pi, scripts), Code sessions, and in-app Chat, filterable by source. Each row shows time, harness, model, status, duration, time-to-first-token, prompt→completion tokens, and tokens/sec; clicking a row opens a Params / Request / Response detail drawer. Sampling params (temperature, top_p, top_k, min_p, max_tokens, penalties, seed, stop, reasoning_effort, response_format, tool_choice) and metadata are always captured; full prompt/response TEXT is opt-in via "Log prompts and responses" (off by default) — reachable from this panel\'s own gear menu or from Settings → Privacy & telemetry, either one patches the same daemon setting. Everything here is in-memory only on this machine (capped by count and by bytes) — never written to `turbollm.db`, never sent to telemetry, and gone on daemon restart; an Export JSONL button in the gear menu is the only way to keep a copy. The gear menu also has per-column visibility toggles and Clear log.\n' +
  'Bottom half: the same live CPU/RAM/GPU gauges and sparklines as Settings → System — same component, same data, so the two screens can never disagree.\n\n' +
  '## Android app\n\n' +
  ...
  '- **OpenAI-compatible**: `POST /v1/chat/completions`, `GET /v1/models`, `POST /v1/embeddings`, `POST /v1/classify`, `POST /v1/rerank` (Jev models)\n' +
  ...
  '## External Chat API\n\n' +
  'A separate, **stateful**, **EXPERIMENTAL** public API at `/api/ext/v1` — distinct from the stateless Gateway above. Off by default; enable with `api.ext.enabled: true` in config.json (whole surface 404s while off; the daemon logs a warning at startup while it\'s on). Docs: turbollm.dev/docs/api/external.\n' +
  '- **Known limitation**: a remote tenant\'s generations currently run under the LOCAL install\'s own tool permissions (toolPolicies/autoAllowAll), not an independent trust boundary — including run_code if it\'s allow-policy on this install.\n' +
  '- **Auth**: Bearer/`X-Api-Key` token resolving to a `tenant` + scopes (`chats:read`/`chats:write`/`runs:write`), added directly to config.json\'s `apiKeys` array (SHA-256 hash only, never the raw key). No key-minting UI yet, and a key resolving to the `local` tenant (e.g. any legacy key with no `tenant` field) is refused with 403 `tenant_not_supported` — `local` is the desktop UI\'s own data.\n' +
  '- **Chats/messages**: full CRUD, cursor-paginated, scoped by `tenant` AND a caller-supplied `owner` (lets one tenant key serve many end users) — cross-tenant or cross-owner access always 404s, never 403 (the only 403s on this surface are `insufficient_scope` and the local-tenant refusal above, both about what the KEY may do, never whether a resource exists).\n' +
  ...
  '## Troubleshooting\n\n' +
  '**Model won\'t load**: Check the VRAM estimate bar in the Model Detail panel — if it\'s full, reduce `ngl` or `ctx`, or choose a smaller/more-quantized model. Verify the active engine is compatible with the model format (GGUF → llama.cpp / TurboQuant / KoboldCpp / llamafile; safetensors → vLLM or MLX). On Windows, vLLM doesn\'t work — use llama.cpp or WSL2. If the load fails outright (crash, or it never finishes starting), a banner now appears at the top of every screen (Chat, Code, anywhere) showing the exact error and a log-tail excerpt, since v1.9.4 — previously that detail only ever showed on the Engines screen. Read that message first; for a very large model on limited memory, "loading" can legitimately take a while before it either succeeds or times out.\n\n' +
  ...
  '**AMD APU / shared-memory GPU shows almost no VRAM**: Fixed in v1.9.2 on Linux — on a unified-memory APU (Strix Halo, Ryzen AI) the GPU\'s shared GTT pool now counts toward the VRAM budget, not just the small BIOS carve-out, so the fit bar and auto-tune see the real number. Discrete cards are deliberately unchanged: their shared pool sits across PCIe and would be slow. Update TurboLLM if on an older version.\n\n' +
  '**AMD APU (Strix Halo, Ryzen AI) + ROCm: large model hangs on load**: Fixed in v1.9.9 — an upstream llama.cpp/ROCm issue on unified-memory APUs could hang the load of a large enough model. `--no-mmap` is now added automatically for this hardware/engine combination instead of requiring a manual "Extra command-line flags" edit. Since v1.12.7 there is also an explicit "Disable memory-mapping" toggle under the model\'s Advanced settings if you want it on other hardware; the flag is still only ever passed once. Update TurboLLM if on an older version; Vulkan is also a valid alternative build for this hardware if you\'d rather switch engines.\n\n' +
  '**VRAM estimate looks too high on a laptop with both an iGPU and a graphics card**: Fixed in v1.9.2 — an integrated GPU\'s budget is a slice of system RAM, so adding it to a real card\'s VRAM double-counted that RAM. The iGPU is now excluded from the total whenever a real card of the same vendor is present, and still counted in full when it is the only GPU.\n\n' +
  ...
  '**Engine update says "up to date" incorrectly**: Fixed in v1.0.0. Update TurboLLM itself if on an older version.\n\n**Docker / headless container: how do I get an API key to log in?** The container publishes port 6996 on `0.0.0.0`, so the daemon requires an API key from the moment it starts and there is no browser inside the container to sign in with. As of v1.12.2 both official Dockerfiles launch with `--print-token`, which mints a fresh key and prints it on every start — grab it from `docker compose -f docker-compose-nvidia.yaml logs | grep \'Token:\'` (or `docker-compose-amd.yaml`) and paste it into the login prompt, or send it as `X-TurboLLM-Auth: <key>`. Each restart mints a new one, so re-run the logs command after a restart. If other devices on your network cannot reach it because the container only shows an internal address, set `TURBOLLM_ADVERTISED_HOST` to the address they should use.\n\n' +
